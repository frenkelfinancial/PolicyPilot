...
// hung up below — belt-and-suspenders so ending a session always debits the
// wallet for its last in-progress call's minutes even if the webhook is
// delayed or dropped.
serve(async (req) => {
  ...
  if (!["pending", "dialing", "connected"].includes(session.status)) {
    return json({ ok: true }); // already ended — nothing to do
  }
  ...
  for (const callControlId of [session.current_call_control_id, session.agent_call_control_id]) {
