...
- **🔴 THE PDF PATH CARRIES THE SAME DATE CHAIN AS THE TABULAR PATH, AND `resolvePdfRowDates()` IS THE ONE PLACE IT LIVES.** For years the fallback above existed only in `applyMapping()`; the PDF branch hard-coded `effectiveDate: null, paidDate: null` and had nothing to fall back to, so a real American-Amicable ledger ingested with **all nine amounts exact and not one date**. The first three hops are byte-identical to the tabular path (`transaction ?? paid ?? effective`) — two definitions of "when did this line happen" one file format apart is the bug class this repo keeps paying for. The PDF path adds **`?? due ?? periodEnd`** because a PDF has no column mapping (no `-1` saying a column is absent) and the period is already on the object. **Do not add those two hops to the tabular path**, which shipped working. The projection itself is `normalizePdfRows()` in `// statement-core`, not in the edge function, so the tests execute the code that ships.
- **🔴 A CARRIER PRINTS SEVERAL DATES AND MAY CALL NONE OF THEM "TRANSACTION DATE".** The ledger has `ACCTG DATE`, `DUE DATE`, `ISSUE DATE`. **The ACCOUNTING date is the transaction date** — it is when the carrier booked the commission, it is on every line, and it keeps a July statement in July. `PDF_SCHEMA` therefore asks for all four dates **by meaning**, naming the synonyms (accounting/acctg/booked/posting/processed), and the deterministic layer picks. Asking for one unlabelled date made the model return the accounting column on some rows and the due column on others, which is worse than nothing because nothing downstream can tell.
- **🔴 THE YEAR COMES FROM THE STATEMENT, NEVER FROM THE CLOCK, AND THE ROLLOVER IS REAL.** This carrier prints `MM-DD` with no year at all. `resolvePartialDate()` resolves it against `statement_date` (then `period_end`) by picking the **nearest** of {anchor−1, anchor, anchor+1}: `12-15` on a `01-31-27` statement is December **2026**, and `08-13` on a `07-31-26` statement stays **2026-08-13** because a due date is legitimately in the future. "Use the statement's year" is wrong once a year for every agent; "on or before" invents a date a year early. **An ISSUE date is deliberately never partial-resolved** — this carrier prints it `MM-YY` (`06-26` = June 2026), indistinguishable from `MM-DD` as a string, so reading it would invent a day. Unresolvable falls through the chain; it is never guessed.
- **🔴 FIRST-YEAR COMMISSION IS `advance`, NEVER `renewal`.** Both `ORDINARY LIFE - INITIAL` and `ORDINARY LIFE - 1ST YEAR` are `advance`. `PDF_SYSTEM` used to define only `chargeback` and left the model to guess the rest, so five first-year lines came back `renewal` on a statement whose own summary read **TOTAL RENEWAL .00**. All six types are now defined in the prompt, and it states that **a section heading governs every line beneath it** — this carrier names the type once, in a heading, never on a line. `normalizeTxnType`'s advance pattern also learned `1st year` / `1st yr` / `1styr` (it knew `first year` and `initial` only); the two are needed together, because the prompt fix serves this carrier and the regex serves the next one.
- **🔴 ON THE PDF PATH, NEGATIVE COMMISSION IS A CHARGEBACK WHATEVER SECTION IT SITS UNDER** — `refineTxnTypeFromText()`, rule 3, and it is a regression that fix caused and caught. Teaching the model that `ORDINARY LIFE - INITIAL` means `advance` made it answer confidently for a −$41.33 line, and a confident answer switches OFF `normalizeTxnType`'s sign tiebreak. The net still reconciled to the cent, so nothing complained, while the line quietly left the Debt tab (`chargeback` + `adjustment` only). A negative **adjustment** still stays an adjustment — a fee is not a reversal. **The tabular path's sign rule is unchanged and must stay so:** there the type column carries the carrier's own transaction word, so "Adjustment" keeps its name whichever way it points; only the PDF path, where the "type" is a product heading, needs the stronger rule.
