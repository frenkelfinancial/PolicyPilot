...
alter table public.billing_config
  ...
  add column if not exists topup_presets_mills jsonb not null default '[5000,10000,25000,50000,100000]'::jsonb;
...
comment on column public.billing_config.topup_presets_mills is
  'Allow-listed wallet top-up amounts in mills, e.g. [5000,10000,25000,50000,100000] = $5/$10/$25/$50/$100. Single source of truth read by both stripe-create-checkout (server-side validation) and the app.html "Add funds" buttons (Phase 1) — change this row, not code, to adjust presets.';
