...
- **Existing digests to match:** `supabase/functions/weekly-digest/` and `daily-digest/` already use the gather (`index.ts`) + render (`email.ts`) split and call Resend with secrets `RESEND_API_KEY`, `DIGEST_FROM`, `DASHBOARD_URL`. Match this structure and reuse the secrets/pattern.
...
3. **Install the Edge Function(s)** containing the gather + render modules and the Resend send logic, plus the **token-based unsubscribe function**. Wire secrets via `supabase secrets set` (Resend API key, service-role key — auto-injected, don't hardcode; a signing secret for the unsubscribe token). Never hardcode keys. Tell me exactly which secrets to set and the commands.
...
8. When done, give me a short checklist of the only manual steps left for me: Resend domain/DKIM verification, any DNS records (and your recommendation on root vs. `reports.frenkelfinancial.com` subdomain), and setting the secrets — with exact values/commands filled in wherever you can.
